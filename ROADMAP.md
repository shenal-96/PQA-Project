...
- **M3 ✅ Reports** — host-side report builder: editable `.docx` (python-docx), self-contained **HTML**, and **PDF via headless Chromium/Edge** (`--headless=new --print-to-pdf`, no LibreOffice/WeasyPrint); hi-DPI matplotlib report images.
  ...
  - **M3.1 ✅ Streamlit report-workflow parity** — brings the legacy `app.py` "Generate Report" UX to the desktop:
    - **Persistent Word-template library** (`desktop/template_store.py`): multi-`.docx` upload, listed with size + `{{Snapshot_N}}` slot count + remove (✕), dropdown select. Stored in the durable app-data dir (`usage_log.data_dir()/templates`) so templates survive restarts/reinstalls (unlike Streamlit's ephemeral `uploads/`). Bridge: `HostBridge.list_templates`/`save_template`/`delete_template`; `build_report` resolves a template by name (`docx_template_name`) as well as inline bytes; `resolve()` is traversal-safe.
    - **Download-format presets** dropdown (PDF / HTML / HTML+PDF / Word / Word+PDF) replacing the loose output checkboxes; **Available placeholders** reference; **pre-flight checks** (snapshot-placeholder completeness vs detected events, and a not-recovered gate with "Remove warnings from report" → `clear_not_recovered` zeroes the flags before snapshot render in `viz_report`); session **generated-reports list** with re-download.
    - Intentionally **no HTML-template editor** (Word-template-centric per owner); the built-in HTML report still drives PDF/HTML output. Backend seam gains `listTemplates?`/`saveTemplate?`/`deleteTemplate?`.
  ...
  - Tests: `tests/test_report.py` + `tests/test_template_store.py` (green). The Chromium subprocess wiring is covered with a fake browser; the **real Chromium/Edge render is Windows/Edge-verifiable** (confirmed locally against a Chromium build — valid multi-page PDF).
...
- **Steady-state (ISO 8528-5 δ bands) ✅** — opt-in evaluation of generator stability during the **stable dwell periods** (between load steps), distinct from transient recovery. Engine: `core/analysis.analyze_steady_state` / `detect_steady_windows` / `evaluate_steady_window` + `AnalysisConfig.steady_*` fields; uses the tight δU/δf bands on raw `df_proc` (never `df_interp`, never the α/β recovery bands), flags governor/AVR **hunting** as a qualitative red flag, and trims a transient-exclusion margin off each dwell. Hybrid confirm flow: `HostBridge.run_analysis` attaches a `steady` contract key when enabled; `HostBridge.recalc_steady` re-evaluates user-edited/labelled dwell windows. Frontend: sidebar toggle "Evaluate steady-state (ISO 8528-5 δ bands)" + `SteadyStatePanel.svelte` (editable dwell table, re-evaluate / reset-to-auto). Tests: `tests/test_steady_state.py`. Off by default → existing contract unchanged.
